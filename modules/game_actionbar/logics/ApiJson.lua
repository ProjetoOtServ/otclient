...
-- === SISTEMA DE PERSISTENCIA DE COORDENADAS DAS BARRAS ===
-- === SISTEMA DE PERSISTENCIA DE COORDENADAS DAS BARRAS ===
function ApiJson.saveBarPosition(barIndex, pos)
    if not pos then return end
    local posNode = {x = pos.x, y = pos.y}
    g_settings.setNode("actionBarPos_" .. barIndex, posNode)
    g_settings.save()
end

function ApiJson.getBarPosition(barIndex)
    local posNode = g_settings.getNode("actionBarPos_" .. barIndex)
    if posNode and posNode.x and posNode.y then
        return {x = posNode.x, y = posNode.y}
    end
    return nil
end

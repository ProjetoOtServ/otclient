...
-- =============================================================
-- AoE — matrizes relativas ao jogador virado para NORTE (y negativo = frente)
-- Formato: { { x = 0, y = -1 }, ... } → g_map.getEnemiesInArea(...)
-- =============================================================
...
-- Anel externo 5x5 (sem o quadrado 3x3 central) — “cruz grossa” / área expandida
...
-- Chaves em minúsculas (use normalizeSpellWords ao buscar)
...
-- Referência pública: mesma tabela (minúsculas nas chaves)
...
-- =============================================================
-- PERSISTÊNCIA — g_settings (Per-Character)
-- =============================================================
...
function BotHelper.captureHotkey(tabId)
  ...
  handler = function(widget, keyCode, keyMods)
    ...
    BotHelper.showGameMessage(
      ...
      ' → ' .. (BotHelper.TAB_MAP[capturedTab] and BotHelper.TAB_MAP[capturedTab].label or capturedTab)
    )
  ...
  if now >= lastPotionTime + POTION_COOLDOWN_HEAL then
    ...
    for i = 1, 2 do
      ...
      if p and p.itemId and p.itemId > 100 then
        ...
        -- LOG TEMPORARIO: Diagnostico de pocoes
        g_logger.info(string.format("[BotHeal] HP: %d%% | Thresh: %d%% | ItemId: %d", hpPct, p.pct, p.itemId))
      end
    end
...
function BotHelper.Healing.setPotion(slotIndex, itemId)
  ...
  if BotHelper.window then
    ...
    if icon then
      local uiItem = icon:getChildById('potionItem')
...
function BotHelper.Healing.assignPotionEvent(slotIndex, btnWidget)
  ...
  mouseGrabberWidget.onMouseRelease = function(self, mousePos, mouseButton)
    ...
    if not isAllowedPotion(itemId) then
      if modules.game_textmessage then
...
function BotHelper.SpellCaster.process(now, player)
  ...
  for _, spellCfg in ipairs(activeSpells) do
    ...
    if normWords ~= '' and manaPct >= spellCfg.manaPct then
      ...
      -- Fallback: se a area retornar 0 ou nao houver matrix, usa o contador geral de criaturas visiveis
      if hitCount == 0 then
        hitCount = countVisibleCreatures()
      end
...
function BotHelper.syncScheduler()
  ...
  -- Limpeza prévia no motor nativo
